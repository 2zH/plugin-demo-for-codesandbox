(function (global, factory) {
  ...
  (factory((global['@pluginjs/input-mask'] = {})));
  ...
  if (!window.Pj) {
    window.Pj = _extends$2({}, envParamters, {
      emitter: new Emitter(),
      plugins: {},
      instances: {},
      get windowWidth() {
  ...
  function register(name) {
    ...
    return function (plugin) {
      ...
      Pj.plugins[name] = Object.assign(plugin, _extends$2({
  ...
  var getRange = curry(function (blocks, index) {
    if (!index) {
  ...
  var computeLens = compose(map(reduce(sum)), Model.of);

  // baseFormat :: config => String
  var baseFormat = curry(function (_ref, data) {
    var blocks = _ref.blocks,
        delimiter = _ref.delimiter;

    var range$$1 = getRange(blocks);
    var blockLens = computeLens(blocks).join();
    return blocks.map(function (v, k) {
      return data.slice.apply(data, toConsumableArray(range$$1(k)));
  ...
  var lensLimit = curry(function (length, data) {
  ...
  var getNumberByStrRange = curry(function (data, min, max) {
  ...
  var dateLimit = function dateLimit(data) {
    ...
    var range$$1 = getNumberByStrRange(data);
    var mm = range$$1(4, 6);
    var dd = range$$1(6, 8);
    // check isNumber
    if (!isNumber(data)) {
  ...
  var timeLimit = curry(function (hourFormat, data) {
    var type = ['AM', 'PM'];
    var time = data.slice(0, 4);
    var range$$1 = getNumberByStrRange(time);
    var min = range$$1(0, 2);
    var sec = range$$1(2, 4);
  ...
  var InputMask = (_dec = eventable(events), _dec2 = styleable(classes), _dec3 = stateable(), _dec4 = register(namespace, {
    defaults: defaults$1,
    methods: methods
    ...
    function InputMask(element) {
      ...
      addClass(_this.classes.INPUT, _this.element);
      _this.data = '';
      _this.initStates();
      _this.initialize();
      return _this;
    ...
    createClass(InputMask, [{
      ...
      value: function bind() {
        ...
        compose.apply(undefined, toConsumableArray([{ type: this.eventName('touch'), handler: function handler() {
            return false;
          ...
          handler: function handler() {
            return _this4.trigger(events.FOCUS);
          }
        }, {
          type: this.eventName('blur.keyboard'),
          handler: function handler() {
            return _this4.trigger(events.BLUR);
          }
        }].map(function (options) {
          return bindEvent(options);
        })))(this.element);
      }
    }, {
      key: 'unbind',
    ...
    return InputMask;
  }(Plugin)) || _class) || _class) || _class) || _class);
