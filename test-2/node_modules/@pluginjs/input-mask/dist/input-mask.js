(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('@pluginjs/utils'), require('@pluginjs/component'), require('@pluginjs/classes'), require('@pluginjs/events'), require('@pluginjs/pluginjs')) :
  typeof define === 'function' && define.amd ? define(['exports', '@pluginjs/utils', '@pluginjs/component', '@pluginjs/classes', '@pluginjs/events', '@pluginjs/pluginjs'], factory) :
  (factory((global['@pluginjs/input-mask'] = {}),global['@pluginjs/utils'],global['@pluginjs/component'],global['@pluginjs/classes'],global['@pluginjs/events'],global['@pluginjs/pluginjs']));
}(this, (function (exports,utils,Component,classes,events,pluginjs) { 'use strict';
  ...
  var map = utils.curry(function (f, functor) {
    return functor.map(f);
  });

  var chain = utils.curry(function (f, monad) {
    return monad.chain(f);
  });

  var reduce = utils.curry(function (f, v) {
    return v.reduce(f);
  });

  var getRange = utils.curry(function (blocks, index) {
    if (!index) {
  ...
  var computeLens = utils.compose(map(reduce(sum)), Model.of);

  // baseFormat :: config => String
  var baseFormat = utils.curry(function (_ref, data) {
  ...
  var lensLimit = utils.curry(function (length, data) {
  ...
  var getNumberByStrRange = utils.curry(function (data, min, max) {
  ...
  var timeLimit = utils.curry(function (hourFormat, data) {
    var type = ['AM', 'PM'];
  ...
  var InputMask = (_dec = pluginjs.eventable(events$1), _dec2 = pluginjs.styleable(classes$1), _dec3 = pluginjs.stateable(), _dec4 = pluginjs.register(namespace, {
    defaults: defaults,
    methods: methods
  }, info), _dec(_class = _dec2(_class = _dec3(_class = _dec4(_class = function (_Component) {
    inherits(InputMask, _Component);
    ...
    function InputMask(element) {
      ...
      _this.options = utils.deepMerge(defaults, options, _this.getDataOptions());
      _this.initClasses(classes$1);
      classes.addClass(_this.classes.INPUT, _this.element);
      _this.data = '';
      _this.initStates();
      _this.initialize();
      return _this;
    ...
    createClass(InputMask, [{
      ...
      value: function initialize() {
            ...
            case 'time':
              {
                ...
                return utils.compose(timeLimit(_format), lensLimit(timeLens));
              }
            case 'date':
              {
                return utils.compose(dateLimit, lensLimit(lens));
              }
            default:
              {
        ...
        var formatMiddleware = function formatMiddleware(options) {
          ...
          switch (type) {
            case 'time':
              {
                return utils.compose(timeFormat, baseFormat(options));
              }
            default:
              {
        ...
        // update :: String -> monad
        var update = utils.compose(chain(updateMiddleware(options)), input);

        // getFormattedData :: {} -> String
        var format = utils.compose(chain(formatMiddleware(options)), Model.of);
        this.onChange = function (event) {
          var data = event.target.value.slice(_this2.lastValue.length);
          _this2.data = update(data);
          _this2.element.value = format(_this2.data);
        };
        this.observe();
      ...
      value: function observe() {
        ...
        utils.compose(events.bindEvent({ type: 'keydown', handler: getLastValue }), events.bindEvent({ type: 'input', handler: this.onChange }))(this.element);
      ...
      value: function unObserve() {
        if (this.hasObserve) {
          events.removeEvent('input', this.element);
      ...
      value: function bind() {
        ...
        utils.compose.apply(undefined, toConsumableArray([{ type: this.eventName('touch'), handler: function handler() {
            return false;
          } }, { type: this.eventName('click'), handler: function handler() {
            return false;
          } }, {
          type: this.eventName('focus.keyboard'),
          handler: function handler() {
            return _this4.trigger(events$1.FOCUS);
          }
        }, {
          type: this.eventName('blur.keyboard'),
          handler: function handler() {
            return _this4.trigger(events$1.BLUR);
          }
        }].map(function (options) {
          return events.bindEvent(options);
        })))(this.element);
      }
    }, {
      key: 'unbind',
      value: function unbind() {
        events.removeEvent(this.eventName());
        this.unObserve();
      }
    }, {
      key: 'enable',
